function cart = cfu_beam2cart_coord (r, zy, zx)
%
% cartesian = cfu_beam2cart_coord (r, zy, zx)
%
...
% Version 1.3 2013-09-17 MFR: Renamed to cfu_beam2cart_coord.
%
...
if min(dimR) > 1,  error('r must be scalar or vector'); end
if min(dimZY) > 1, error('zy must be scalar or vector'); end
if min(dimZX) > 1, error('zx must be scalar or vector'); end
