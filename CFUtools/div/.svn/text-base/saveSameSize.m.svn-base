...
%
%        'dpi'         Sets the DPI to use. Settings this variable breaks the purpose
%                      of this function, since the resulting image will no longer have
%                      the same resolution as the figure.
%                      It defaults to 0, meaning same resolution as the figure.
%
%   Example usage:
...
function param_settings = processArgs(h, varargin) 
    ...
    param_settings.dpi      = '-r0'; %0=screen resolution
    for i = 1 : 2 : length(varargin)-1 
        ...
        % caller specified dpi to use
        if ischar(param_arg) && strcmpi(param_arg, 'dpi')
            val_arg = varargin{i+1};
            if isnumeric(val_arg) && val_arg >= 0
                param_settings.dpi = sprintf('-r%.0f',val_arg);
            else 
                error('saveSameSize:InvalidDPI', ...
                      'saveSameSize invalid DPI value specified');
            end
        end
