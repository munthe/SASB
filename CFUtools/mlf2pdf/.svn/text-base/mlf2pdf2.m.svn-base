...
function mlf2pdf2(h,FileName,Packages,varargin)
...
% LaTeX Command
%-------------------------------------------------------
str_eps=sprintf('latex -shell-escape --src -interaction=nonstopmode %s2.tex',TempName);
str_pdf=sprintf('pdflatex -shell-escape --src -interaction=nonstopmode %s2.tex',TempName);
disp(sprintf('\n[LaTeX Command] %s',str_pdf));
[hdos,wdos]=system(str_eps);
fprintf('%s', wdos);
[hdos,wdos]=system(str_pdf);
fprintf('%s', wdos);
