...
function mlf2pdf(h,FileName,Packages,varargin)
...
matlabfrag(TempName,'handle',h,varargin{:});  %call matlabfrag to export figure to .eps and .tex file.
...
%fprintf(fid,'\\usepackage[utf8latin1]{inputenc}\n');
fprintf(fid,'\\usepackage[utf8]{inputenc}\n');
fprintf(fid,'\\usepackage{graphicx}\n');
fprintf(fid,'\\usepackage{amsmath,mathtools}\n');
fprintf(fid,'\\usepackage{amssymb,amsfonts}\n');
...
if isunix==0
   ...
   mv_err = dos(sprintf('ren %s.pdf %s.pdf',TempName,FileName)); 
else 
    if exist(sprintf('rm %s.pdf',FileName))
        unix(sprintf('rm %s.pdf',FileName));
    end
    mv_err =unix(sprintf('mv %s.pdf %s.pdf',TempName,FileName));
end
if mv_err
    error('Error: Output file did not exist. Check the LaTeX log for the source of the error.');
end
